...
const UniversityIndexPage = () => {
  ...
  const { data, error, mutate } = useSWR(
    `http://schooliu.ddns.net:3000/courses/${router.query.university}?page=1&numberPerPage=50`,
    fetcher
  );
  ...
  const addNewSubject = async ({ name }: { name: string }) => {
    ...
    const res = await fetch("http://schooliu.ddns.net:3000/courses", {
      method: "POST",
      body: JSON.stringify({
        name: name,
  ...
  return (
    <>
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <AddSubjectModal onAdd={addNewSubject} />
      </div>
    </>
  );
};
