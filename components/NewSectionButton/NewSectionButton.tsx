...
export const NewSectionButton: FunctionComponent<NewSectionButtonProps> = ({blocksWhereToAdd}) => {
  ...
  return (
    <div onBlur={ () => setExpanded(false) }>
      { expanded ? <div className="space-x-4">
        <button>Text</button>
        <button onClick={addBlock}>Theorem</button>
        <button>Exemple</button>
      </div> : <button onClick={ () => setExpanded(true) }>Add object</button> }
    </div>
  );
};
